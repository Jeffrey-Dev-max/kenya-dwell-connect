...
-- First drop all policies that depend on agency_id

-- Drop all policies that reference agency_id
...
-- Now we can safely remove agency_id column from properties table
...
-- Update app_role enum to only include tenant and homeowner roles
...
-- Drop member_role enum since it's no longer needed
DROP TYPE IF EXISTS public.member_role CASCADE;

-- Recreate simplified RLS policies for properties (without agency references)
...
-- Recreate simplified RLS policies for property_media (without agency references)  
